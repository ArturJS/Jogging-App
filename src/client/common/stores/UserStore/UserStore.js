...
class UserStore {
  ...
  async init(userData) {
    if (__SERVER__ && userData) {
      ...
      const initialUserData = _.get(window, '__INITIAL_APP_STATE__.stores.userStore');
      // (initialUserData === null) - this is flag from server side if user is NOT authorized
      if (initialUserData) {
        this._setUserData(initialUserData);
      }
      else if (initialUserData !== null) { // in case of static html export

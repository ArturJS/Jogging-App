...
const isHerokuEnvironment =
    process.env.NODE && process.env.NODE.indexOf('heroku') >= 0;

if (!isHerokuEnvironment) {
    console.info(
        '"heroku-build" task skipped (due to non heroku environment)...'
    );
    process.exit(0);
}
...
const herokuBuild = spawn(
    `npx${extension}`,
    ['npm-run-all', '-s', 'build', 'create-db-prod', 'db:migrate'],
    {
        cwd: projectPath
    }
);
